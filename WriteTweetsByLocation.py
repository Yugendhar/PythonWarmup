...
def setPath():
    ...
    if isdev==1 :
        ...
        outpath = 'g:\SC_Flood_Data\Json_OutPut_Location\\'
...
def GetStateDateTextFile(curstate,curdate):
    ...
    if date_filename not in file_txt_objects.keys():
        file_txt_objects[date_filename]=TextFile(date_filename, 'a')
    return file_txt_objects[date_filename]
...
# Store Extracted Data from Json files into text file
# All required data
def printExtractedDataToTextWithLocation(txtObj,location) :
    count= txtObj.increment
    body_text = body.replace('\n',' ').replace('\r',' ').replace('\r\n',' ').replace('\n\r',' ')
    txtObj.txtfileObj.write(str(count)+','+str(count)+','+location+','+body_text+'\n')
'''
=========================================================
            ...
            if "gnip" in data:
                ...
                if country:
                    if country==unitedStates:
                        ...
                        printExtractedDataToTextWithLocation(unitedStatestxtfile,region)
                        if region==state:
                            ...
                            printExtractedDataToTextWithLocation(nonStatetxtfile,region)
                            if objectpostedtime[:10] > fromdate and  objectpostedtime[:10]< todate:
                                printExtractedDataToTextWithLocation(GetStateDateTextFile(nonState,objectpostedtime[:10]),region)
                    else :
                        # Non United States Countries
                        printExtractedDataToTextWithLocation(nonUnitedStatesTextFile,country)
                else :
